...
export const ReportsView: React.FC<ReportsViewProps> = ({
  ...
  const handleReTranscribeVideo = async (qId: string) => {
    ...
    try {
      ...
      // Ask the server to transcribe the durable recording by ID. The browser
      // should not download and re-encode the entire video just to request AI
      // transcription, especially after a Render restart.
      const res = await api.gemini.transcribeVideo({
        videoId: response.videoRecording.id,
        mimeType: response.videoRecording.mimeType || 'video/webm',
